...
def simpleTokenize(string):
    ...
    """
    #return <FILL IN>
    #filter list of tokens with evalueate in boolean. '' evalueates to false and is removed 
    return filter(bool,re.split(split_regex,string.lower()))
...
def tokenize(string):
    ...
    #return <FILL IN>
    string=string.lower()
    temp = filter(bool,re.split(split_regex,string))
...
a10=amazonSmall.take(3)
g10=googleSmall.take(3)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #prwta sortarw kai meta pairnw to prwto
...
def tf(tokens):
    ...
    new_dict.update((x, y/tokenslen) for x, y in new_dict.items())
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    # dict with values {'brown': 0.16666666666666666, 'lazy': 0.16666666666666666,..}
    tfIdfDict =tfs
    # find from total idfs the value for key x and update dictionary
    tfIdfDict.update((x, y*idfs.get(x)) for x, y in tfIdfDict.items())
    return tfIdfDict
...
#or .map(lambda r : computeSimilarity(r))
...
# TODO: Replace <FILL IN> with appropriate code
sims = sims = similaritiesBroadcast.map(lambda (id_google, id_amazon, cs): (id_amazon + ' ' + id_google, cs))

trueDupsRDD = (sims
               .join(goldStandard)
               .map(lambda(id_composite, value): (value[0])))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.mean()

nonDupsRDD = (sims
               .leftOuterJoin(goldStandard)
               .filter(lambda(id_composite, value): value[1] == None)
               .map(lambda(id_composite, value): (value[0]))
              )
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map( lambda (key, tfidfDict): (key, norm(tfidfDict))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map( lambda (key, tfidfDict): (key, norm(tfidfDict))).collectAsMap()
...
def invert(record):
    ...
    ID = record[0]
    tokens = record[1]
    pairs = [(item, ID) for item in tokens]
    return (pairs)
