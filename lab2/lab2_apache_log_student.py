...
#an evaza anti s[0] s tha gyrisei kai count : u'slip3.nilenet.com', 11)
...
# TODO: Replace <FILL IN> with appropriate code makis

dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.day,log.host,1))
#gia na einai monadiko
distinctTriples = dayToHostPairTuple.distinct() 
#peta eksw to host kai krataw eggrafes tis morfis imera,1
distinctTwoples=distinctTriples.map(lambda (a,b,c): (a,1))
#kane to athroisma
dayGroupedHosts = distinctTwoples.reduceByKey(lambda a, b : a + b)

dayHostCount = dayGroupedHosts.count()
dailyHosts = dayGroupedHosts.sortByKey().cache()
dailyHostsList = dailyHosts.take(30)
print 'Unique hosts per day: %s' % dailyHostsList

#####Enallaktiki lisi me .groupByKey().mapValues(len)
#dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.day,log.host))
##to apotelesma einai [(1, 2582), (3, 3222), posoi diaforetikoi hosts kathe mera
#dayGroupedHosts = dayToHostPairTuple.distinct().groupByKey().mapValues(len).sortByKey()
#dailyHosts=dayGroupedHosts.cache()
#dailyHostsList = dailyHosts.collect()
#print 'Unique hosts per day: %s' % dailyHostsList
#print dayGroupedHosts.take(10)
...
dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.day,1))
dailyHostHits = dayToHostPairTuple.reduceByKey(lambda a, b : a + b)
joined=dailyHosts.join(dailyHostHits)
groupedByDay=joined.map(lambda (a,(b,c)) :(a,c/b) )
sortedByDay = groupedByDay.sortByKey()
avgDailyReqPerHost = (sortedByDay.cache())
avgDailyReqPerHostList = avgDailyReqPerHost.take(30)
print 'Average number of daily requests per Hosts is %s' % avgDailyReqPerHostList

#print 'Unique hosts per day: %s \n'  % dailyHostsList
#print 'Total hosts per day: %s \n' % dailyHostHits.sortByKey().take(5)
#print 'Total hosts per day count: %s \n' % dailyHostHits.count()
#print  'Joined hosts per day: %s \n ' % joined.sortByKey().take(5)
#print  'Join count: %s \n ' % joined.count()
...
##to apotelesma einai [(1,33996),...] diladi mera kai total hits
#groupedByDay = dayAndHostTuple.groupByKey().mapValues(len).sortByKey()

##join me to apo panw posoi unique hosts ana mera
#joined=dailyHosts.join(groupedByDay)
#sortedByDay = joined.sortByKey()

##diairese total hits/num of hosts gia na pareis average
#avgDailyReqPerHost=sortedByDay.map(lambda (host,(distinct,total)) : (host,total/distinct) ).cache()
#avgDailyReqPerHostList = avgDailyReqPerHost.take(30)
#print sortedByDay.take(10)
#print 'Average number of daily requests per Hosts is %s' % avgDailyReqPerHostList
...
errDateSorted = errDateSum.sortBy(lambda x: x[0]).cache()
