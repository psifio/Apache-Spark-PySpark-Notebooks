...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    mysum =sum(ratings)
    averageRating=(1.0*mysum)/ratingscount
    return (movieid,(ratingscount,averageRating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (movied,(mname,(count,avg))):(avg,mname,count) ))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg,mname,count):count>500)
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (a,b,c):((a,b),c) )    

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (a,b,c):((a,b),c) )

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda a: (a[0], math.pow(a[1][0] - a[1][1],2) ))
                       )
                        
    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda a: a[1]).sum()
...
validationForPredictRDD = validationRDD.map(lambda (movieid,userid,rating):(movieid,userid))
...
myModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda (movieid,userid,rating):(movieid,userid))
...
#or trainingAvgRating = trainingRDD.map(lambda mytuple: mytuple[2]).mean()
trainingAvgRating = ( trainingRDD
                        .map(lambda (movieid,userid,rating):rating)
                        .reduce(lambda a,b:a+b)
                    )/trainingRDD.count()

print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (movieid,userid,rating):(movieid,userid,trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                      .subtract(myRatingsRDD))

#print moviesRDD.take(2)
#print
#print myRatingsRDD.take(2)
...
movieCountsRDD = (movieIDsWithAvgRatingsRDD
                   .map(lambda (MovieID, (numberOfRatings,averageRating)):(MovieID,numberOfRatings) )
                 )
